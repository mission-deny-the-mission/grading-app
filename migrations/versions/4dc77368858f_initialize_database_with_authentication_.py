"""Initialize database with authentication models

Revision ID: 4dc77368858f
Revises: 
Create Date: 2025-11-15 02:45:39.402197
...
def downgrade():
    ...
    sa.Column('display_order', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.Column('total_possible_points', sa.NUMERIC(precision=10, scale=2), autoincrement=False, nullable=False),
    sa.CheckConstraint('display_order > 0', name=op.f('ck_question_order')),
    sa.ForeignKeyConstraint(['scheme_id'], ['grading_schemes.id'], name=op.f('scheme_questions_scheme_id_fkey')),
    sa.PrimaryKeyConstraint('id', name=op.f('scheme_questions_pkey')),
    sa.UniqueConstraint('scheme_id', 'display_order', name=op.f('uq_question_order'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
