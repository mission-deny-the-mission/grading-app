...
def test_remove_job_from_wrong_batch(client, app):
    """Test removing job from wrong batch returns error."""
    with app.app_context():
        from models import GradingJob, JobBatch, db
...
def test_add_jobs_to_batch_skips(client, app):
    """Test adding jobs to batch skips already assigned and missing jobs."""
    with app.app_context():
        from models import GradingJob, JobBatch, db
...
def test_export_empty_batch_zip(client, app):
    """Test exporting empty batch creates empty ZIP file."""
    with app.app_context():
...
def test_get_provider_models_unknown(client):
    """Test getting models for unknown provider returns error."""
...
def test_delete_saved_marking_scheme(client):
    """Test deleting saved marking scheme."""
    # Create via JSON
    resp = client.post(
