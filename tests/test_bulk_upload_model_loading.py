...
class TestBulkUploadModelLoading:
    ...
    def test_bulk_upload_page_no_references_to_missing_elements(self, client):
        ...
        for forbidden_element in forbidden_elements:
            assert (
                forbidden_element not in element_references
    ...
    def test_javascript_basic_error_handling(self, client):
        ...
        for function_name in critical_functions:
            ...
            has_error_handling = any(
                re.search(pattern, script_content, re.DOTALL) for pattern in error_handling_patterns
            )
    ...
    def test_all_javascript_elements_exist_in_dom(self, client):
        ...
        assert len(missing_elements) == 0, f"JavaScript references elements that don't exist in DOM: {missing_elements}"
    ...
    def test_critical_javascript_functions_exist(self, client):
        ...
        for function_name in required_functions:
            assert function_name in functions, f"Missing critical JavaScript function: {function_name}"
    ...
    def test_javascript_load_available_models_function_structure(self, client):
        ...
        assert (
            "catch(" in function_section or "try" in function_section
    ...
    def test_javascript_load_all_providers_models_function_structure(self, client):
        ...
        # Check that function references correct elements
        required_references = ["getElementById('allModelsContainer')", "getElementById('providerFilters')"]
        ...
        assert (
            "provider-filter" in function_section
        ), "Missing provider filter creation in loadAllProvidersModels function"

        # Check that function creates model checkboxes
        assert (
            "unified-model-checkbox" in function_section
        ), "Missing model checkbox creation in loadAllProvidersModels function"
    ...
    def test_javascript_event_listeners_are_properly_bound(self, client):
        ...
        assert "addEventListener('change', function()" in html_content, "Missing provider change event listener"
        assert "loadAvailableModels()" in html_content, "Missing loadAvailableModels() call in event listeners"
    ...
    def test_fallback_models_function_exists(self, client):
        ...
        assert "getBulkFallbackModels(" in html_content, "Missing getBulkFallbackModels function"

        # Check that fallback models are defined for common providers
        providers = ["openrouter", "claude", "gemini", "openai"]
        for provider in providers:
            assert f"'{provider}': [" in html_content, f"Missing fallback models for provider {provider}"
    ...
    def test_no_hardcoded_model_references_that_could_break(self, client):
        ...
        for pattern in problematic_patterns:
            assert pattern not in script_content, f"Found problematic pattern that could cause regression: {pattern}"
    ...
    def test_refresh_models_functionality(self, client):
        ...
        assert "refreshBulkProviderModels()" in html_content, "Missing refreshBulkProviderModels function"

        assert "refreshBulkModelsBtn" in html_content, "Missing refresh button element"

        # Check that refresh function includes cache busting
        assert "?t=${Date.now()}" in html_content, "Missing cache busting in refresh function"
    ...
    def test_model_display_name_function_exists(self, client):
        ...
        assert "getModelDisplayName(" in html_content, "Missing getModelDisplayName function"

        # Check for common model name mappings
        model_mappings = ["anthropic/claude-opus-4-1", "openai/gpt-4o", "anthropic/claude-3-5-sonnet-20241022"]

        for model in model_mappings:
            assert model in html_content, f"Missing model display name mapping for {model}"
    ...
    def test_regression_prevention_popular_models_element_missing(self, client):
        ...
        assert (
            "popularModelsDiv" not in script_content
        ), "Regression: Found reference to 'popularModelsDiv' which should not exist"

        assert (
            "getElementById('popularModels')" not in script_content
        ), "Regression: Found direct reference to 'popularModels' element"

        assert (
            "document.getElementById('popularModels')" not in script_content
    ...
    def test_model_loading_event_listeners_properly_bound(self, client):
        ...
        for listener in required_listeners:
            assert listener in script_content, f"Missing required event listener: {listener}"
        ...
        for trigger in model_loading_triggers:
            assert trigger in script_content, f"Missing model loading trigger: {trigger}"
    ...
    def test_model_loading_error_recovery(self, client):
        ...
        assert has_try_catch or has_promise_catch, "No error handling found in JavaScript code"
        ...
        if load_models_section:
            has_error_handling_in_load = (
                "try" in load_models_section and "catch" in load_models_section
            ) or ".catch(" in load_models_section
            assert has_error_handling_in_load, "loadAvailableModels function should have error handling"

            # Check for some form of user feedback on errors
            has_user_feedback = (
                "status.textContent" in load_models_section
                or "errorDiv" in load_models_section
                or "console.error" in load_models_section
            )
            assert has_user_feedback, "loadAvailableModels should provide user feedback on errors"
    ...
    def test_model_loading_user_feedback_mechanisms(self, client):
        ...
        for pattern in feedback_patterns:
            assert pattern in script_content, f"Missing user feedback pattern: {pattern}"
    ...
    def test_model_loading_performance_optimizations(self, client):
        ...
        for pattern in performance_patterns:
            matches = re.findall(pattern, script_content)
            assert len(matches) > 0, f"Missing performance optimization pattern: {pattern}"
    ...
    def test_model_loading_with_mocked_api_failure(self, mock_get_cached_models, client):
        ...
        if response.status_code == 200:
            ...
            assert (
                "error" in data or "message" in data or "models" in data
    ...
    def test_model_loading_cross_browser_compatibility(self, client):
        ...
        for pattern in compatible_patterns:
            assert pattern in script_content, f"Missing cross-browser compatible pattern: {pattern}"
        ...
        for pattern in incompatible_patterns:
            assert pattern not in script_content, f"Found incompatible browser pattern: {pattern}"
