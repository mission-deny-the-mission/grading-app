...
class TestCeleryWorkerFailures:
    ...
    @pytest.mark.skip(
        reason="Celery-specific test - app now uses ThreadPoolExecutor task queue"
    )
    @patch("tasks.celery")
...
class TestRetryLogicUnderLoad:
    ...
    def test_retry_with_different_errors(self, mock_task):
        ...
        permanent_errors = [ValueError("Invalid data"), TypeError("Type mismatch")]
...
class TestTaskQueueManagement:
    ...
    def test_task_queue_priority(self, mock_apply_async):
        ...
        # High priority task
        high_priority = process_submission_task.apply_async(args=["sub1"], priority=9)

        # Low priority task
        low_priority = process_submission_task.apply_async(args=["sub2"], priority=1)
