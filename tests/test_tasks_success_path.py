"""
Test a successful processing path for a submission using tasks.process_submission_sync.
"""
...
def test_process_submission_sync_success(app, tmp_path):
    ...
    with app.app_context():
        job = GradingJob(job_name='Job Success', provider='lm_studio', prompt='Please grade.')
        db.session.add(job)
        db.session.commit()
