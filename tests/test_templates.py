...
class TestTemplatesRoutes:
    ...
    def test_templates_page_with_filters(self, client):
        ...
        response = client.get("/templates?type=job&category=report&search=grading")
        assert response.status_code == 200
    ...
    def test_create_template_missing_name(self, client):
        ...
        data = {"description": "Template without name", "template_type": "batch"}
    ...
    def test_template_filter_helper_function(self, client, app):
        ...
        with app.app_context():
            ...
            # Test type filtering
            assert _template_matches_filter(batch_template, "batch", None, "")
            assert not _template_matches_filter(batch_template, "job", None, "")
            assert _template_matches_filter(job_template, "job", None, "")
            assert not _template_matches_filter(job_template, "batch", None, "")

            # Test category filtering
            assert _template_matches_filter(batch_template, "all", "essay", "")
            assert not _template_matches_filter(batch_template, "all", "report", "")
            assert _template_matches_filter(job_template, "all", "report", "")
            assert not _template_matches_filter(job_template, "all", "essay", "")
...
class TestTemplateEdgeCases:
    ...
    def test_update_template_with_invalid_data(self, client, app):
        ...
        response = client.put(f"/api/templates/{template_id}", json=invalid_data)
