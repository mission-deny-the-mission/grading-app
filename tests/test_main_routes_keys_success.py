...
and a success case for upload_marking_scheme.
"""
...
class TestApiKeySuccess:
    def test_openrouter_key_success(self, client):
        with patch('routes.main.openai.ChatCompletion.create', return_value=MagicMock()):
            resp = client.post('/test_api_key', json={'type': 'openrouter', 'key': 'ok'})
    ...
    def test_claude_key_success(self, client):
        ...
        with patch('routes.main.Anthropic', return_value=fake):
            resp = client.post('/test_api_key', json={'type': 'claude', 'key': 'ok'})
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data['success'] is True
    ...
    def test_gemini_key_success(self, client):
        ...
        with patch('google.generativeai.configure'), \
            ...
            resp = client.post('/test_api_key', json={'type': 'gemini', 'key': 'ok'})
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data['success'] is True
    ...
    def test_openai_key_success(self, client):
        ...
        with patch('routes.main.openai.OpenAI', return_value=fake_client):
            resp = client.post('/test_api_key', json={'type': 'openai', 'key': 'ok'})
        assert resp.status_code == 200
        data = json.loads(resp.data)
        assert data['success'] is True
...
class TestUploadMarkingSchemeSuccess:
    def test_upload_marking_scheme_success(self, client):
        data = {
            'marking_scheme': (io.BytesIO(b'RUBRIC: 1) Content 2) Structure'), 'rubric.txt'),
            'name': 'Rubric',
