...
class TestExternalChecksSuccess:
    ...
    def test_ollama_success(self, client):
        ...
        fake_get_resp.json.return_value = {"models": [{"name": "llama2"}]}  # A generative model

        # Mock the POST request to /api/generate (successful generation)
        fake_post_resp = MagicMock(status_code=200)

        with patch("requests.get", return_value=fake_get_resp), patch("requests.post", return_value=fake_post_resp):
