...
class TestBatchUIEndpoints:
    ...
    @pytest.mark.api
    @pytest.mark.integration
    def test_add_jobs_to_batch_endpoint(
        self, client, sample_batch, sample_job
    ):
        """Test POST /api/batches/{batch_id}/jobs endpoint."""
        response = client.post(
            f'/api/batches/{sample_batch.id}/jobs',
            json={'job_ids': [sample_job.id]}
        )
        assert response.status_code == 200
    ...
    def test_batch_endpoints_with_invalid_batch_id(self, client):
        ...
        # Test create job endpoint
        response = client.post(
            '/api/batches/invalid-id/jobs/create',
            json={'job_name': 'Test Job'}
        )
        assert response.status_code == 400
