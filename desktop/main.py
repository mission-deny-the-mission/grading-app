        ...
        if last_check:
            try:
                ...
                if datetime.utcnow() - last_check_dt < timedelta(hours=24):
                    logger.debug(
                        "Update check skipped - checked less than 24 hours ago"
                    )
                    return
...
def create_main_window(
    ...
    try:
        ...
        window = webview.create_window(
            title=title, url=url, width=width, height=height, hidden=start_hidden
        )
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            initialize_scheduler()
            logger.info(
                "Scheduler jobs initialized (cleanup_old_files, cleanup_completed_batches)"
            )
        ...
        # Detect pytest environment to avoid spawning extra threads in unit tests
        is_test_env = os.getenv("PYTEST_CURRENT_TEST") is not None

        # Check for updates in background thread (non-blocking)
        if settings.get("updates.auto_check", True) and not is_test_env:
            ...
            logger.debug("Skipping background update check in test environment")
        ...
        window_title = (
            f"Grading App v{__version__}" if not is_test_env else "Grading App"
        )
        window_kwargs = {
            "url": flask_url,
            "title": window_title,
        ...
        if start_minimized:
            window_kwargs["start_hidden"] = True
        window = create_main_window(**window_kwargs)

        # Create system tray if enabled
        tray_icon = None
        if show_tray and not is_test_env:
            ...
            try:
                ...
                if tray_icon:
                    ...
                    tray_thread = threading.Thread(
                        target=tray_icon.run, daemon=True, name="SystemTrayThread"
                    )
                    tray_thread.start()
                    logger.info("System tray icon started in background thread")
                else:
                    logger.warning(
                        "System tray icon creation failed, continuing without tray"
                    )

            except Exception as e:
                logger.error(f"Failed to create system tray: {e}")
                logger.warning("Continuing without system tray functionality")
        elif show_tray and is_test_env:
            logger.debug("Skipping system tray creation in test environment")
